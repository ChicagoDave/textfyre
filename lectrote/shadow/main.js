...
var hintswin = null; /* the hints window, if active */
...
/* Open the hints window. (It must not already exist.)
*/
function open_hints_window()
{
    var winopts = {
        width: 810, height: 600,
        useContentSize: true,
    ...
    window_position_prefs(winopts, 'hintswin');
    if (window_icon)
        winopts.icon = window_icon;

    hintswin = new electron.BrowserWindow(winopts);

    if (process.platform != 'darwin') {
        var template = construct_menu_template('hints');
        var menu = electron.Menu.buildFromTemplate(template);
        hintswin.setMenu(menu);
    }

    hintswin.on('closed', function() {
        hintswin = null;
    });
    hintswin.on('move', window_position_prefs_handler('hintswin', hintswin));
    hintswin.webContents.on('will-navigate', function(ev, url) {
            require('electron').shell.openExternal(url);
            ev.preventDefault();
        });

    hintswin.loadURL('file://' + __dirname + '/shadowhints.html');
}
